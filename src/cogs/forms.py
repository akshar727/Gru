...
class FormEditor(discord.ui.View):
    ...
    async def delete(self, _: discord.ui.Button, interaction: discord.Interaction):
        ch = interaction.channel
        await interaction.response.send_message(ephemeral=True, content="Are you sure?")
        view = ConfirmDeletion(self.form, self.msg_id, await ch.fetch_message(self.msg_id),
                               await interaction.original_message())
        await interaction.edit_original_message(view=view)

    @discord.ui.button(label="View Questions", style=discord.ButtonStyle.gray)
    async def questions(self, _: discord.ui.Button, interaction: discord.Interaction):
        ch = interaction.channel
    ...
    async def responses(self, _: discord.ui.Button, interaction: discord.Interaction):
        self.stop()

    @discord.ui.button(label="Form Open/Closed", style=discord.ButtonStyle.green)
    async def form_toggle(self, _: discord.ui.Button, interaction: discord.Interaction):
    ...
    async def form_edit(self, _: discord.ui.Button, interaction: discord.Interaction):
        form = NewFormTitle(self.msg_id, self.form)
        await interaction.response.send_modal(form)

    @discord.ui.button(label="Back", style=discord.ButtonStyle.blurple)
    async def form_back(self, _: discord.ui.Button, interaction: discord.Interaction):
        ch = interaction.channel
    ...
    async def new(self, button: discord.ui.button, interaction: discord.Interaction):
        ...
        if forms == 5:
            return await interaction.response.send_message(
                "You have the max amount of forms (5)! Remove one to create a new form!")
...
class NewFormTitle(discord.ui.Modal):
    def __init__(self, id, form_name):
        ...
        self._title = discord.ui.TextInput(label="New Title", placeholder="Ex: Staff Application", required=True,
...
class NewForm(discord.ui.Modal):
    def __init__(self, id):
        ...
        self._title = discord.ui.TextInput(label="Title", placeholder="Ex: Staff Application", required=True,
                                           style=discord.TextInputStyle.short)
        self._id = id
        self.add_item(self._title)
...
class Form(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if apps[str(id)]['responses'][str(interaction.user.id)] == {}:
            apps[str(id)]['responses'][str(interaction.user.id)] = {'answers': submission_data, 'accepted': False,
                                                                    'denied': False, 'reason': ''}
        else:
        ...
        if len(self.tmp) > self.step * 5:
            await interaction.send("Click here to go to the next step of the form!",
                                   view=NextButton(self.tmp, interaction.user, self.step, self.name), ephemeral=True)
        else:
            return await interaction.send(
                "You have finished this form! I will message you when your form status has changed.", ephemeral=True)
